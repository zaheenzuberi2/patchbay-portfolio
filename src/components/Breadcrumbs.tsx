    ...
    <>
      ...
      <nav aria-label="Breadcrumb">
        {/* gap-x-1 rather than gap-x-2 because each link now carries its own
            ...
            min-w-11 rather than more padding: padding sized for "Home" still
            left "FAQ" at 41px once the /faq/[slug] pages made FAQ a middle,
            linked crumb instead of the final unlinked one. A floor is exact
            at any word length; padding is a guess that has now missed twice. */}
            ...
            return (
              <li key={c.href} className="flex items-center gap-2">
                {last ? (
                  ...
                  <Link
                    href={c.href}
                    className="flex min-h-11 min-w-11 items-center justify-center px-2 transition-colors hover:text-paper"
                  >
                    {c.name}
                  </Link>
                )}
